...
//  val pathsSortedByHops = ReducibleLazyEvaluation<Pair<Node, Node>, List<PickedPath>>({ pathsSortedStatically[it].sortedBy { it.third.size } })
    ...
    open val fStateMetric: ReducibleLazyEvaluation<Edge, Double> =
        ReducibleLazyEvaluation({ +(it.n1.loc - it.n2.loc) + topo.internalLength })
    ...
    override fun P4() {
        ...
        (pathsSortedDynamically + extraPaths).forEach { (_, width, p) ->
            ...
            p.dropLast(2).zip(p.drop(1).dropLast(1)).zip(p.drop(2)).forEach { (n12, next) ->
                ...
                val prevLinks =
                    n.links.filter { it.entangled && !it.swappedAt(n) && it.contains(prev) }.sortedBy { it.id }
                        .take(width)
                val nextLinks =
                    n.links.filter { it.entangled && !it.swappedAt(n) && it.contains(next) }.sortedBy { it.id }
                        .take(width)
        ...
        pathsSortedDynamically.forEach { (_, width, p) ->
            ...
            (1..width).forEach {
                ...
                while (segmentsToTry.isNotEmpty()) {
                    ...
                    val links =
                        topo.kHopNeighborLinks(src).intersect(topo.kHopNeighborLinks(dst)).filter { it.assigned }
                    ...
                    if (rp.second.isEmpty()) { // nodes only know qubit assignment. cannot decide on the link state
                        ...
                        while (true) { // we don't want this loop. But because nodes do not know states on other recovery paths, they should assume they are the hero
                            ...
                            if (rp.size == 2) { // previous loop is not visited
                                ...
                                val link =
                                    prev.links.filter { it.entangled && it.contains(next) && !(it.s1 && it.s2) && it !in pendIn && it !in pendOut && it !in visited }
                                        .minBy { it.id }
...
class CreationRate(topo: Topo, allowRecoveryPaths: Boolean = true) :
    OfflinePathBasedAlgorithm(topo, allowRecoveryPaths) {
    override val name: String = "CR" + if (allowRecoveryPaths) "" else "-R"
    override val fStateMetric: ReducibleLazyEvaluation<Edge, Double> =
        ReducibleLazyEvaluation({ E.pow(topo.alpha * +(it.n1.loc - it.n2.loc)) })
    override val dynamicComparator = Comparator<PickedPath> { (m1), (m2) ->
        m1.compareTo(m2)
    }
}

open class MultiMetric(topo: Topo, allowRecoveryPaths: Boolean = true) :
    OfflinePathBasedAlgorithm(topo, allowRecoveryPaths) {
...
class GlobalLinkState(topo: Topo, allowRecoveryPaths: Boolean = true) : MultiMetric(topo, allowRecoveryPaths) {
    ...
    override fun P4() {
        ...
        while (true) {
            ...
            if (path.size == 2) {
                path[0].links.filter { it.entangled && !it.swapped() && (it.contains(path[1])) }
                    .forEach { freeValidLinks.remove(it) }
            }

            path.dropLast(2).zip(path.drop(1).dropLast(1)).zip(path.drop(2)).forEach { (n12, next) ->
                val (prev, n) = n12

                val prevLinks = n.links.filter { it.entangled && (it.n1 == prev && !it.s2 || it.n2 == prev && !it.s1) }
                    .sortedBy { it.id }.take(width)
                val nextLinks = n.links.filter { it.entangled && (it.n1 == next && !it.s2 || it.n2 == next && !it.s1) }
                    .sortedBy { it.id }.take(width)
