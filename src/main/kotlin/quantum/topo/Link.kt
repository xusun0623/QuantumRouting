...
class Link(
    ...
    override fun toString(): String {
        return "L#${id.padTo(topo.linkDigits)} ${if (entangled) "✓" else "✗"} ${if (assigned) "[" else "("}${
            n1.id.padTo(
                topo.nodeDigits
            )
        },${n2.id.padTo(topo.nodeDigits)}${if (assigned) "]" else ")"} ${l.format(2, topo.distanceDigits)}km"
    }
